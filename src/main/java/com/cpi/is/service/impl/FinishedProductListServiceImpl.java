...
public class FinishedProductListServiceImpl implements FinishedProductListService {
    ...
    public List<FinishedProductListEntity> getData(Long branchId) throws Exception {
        return finishedProductListDAO.getData(branchId);
    }

    @Override
    public String saveItem(HttpServletRequest request) throws Exception {
        return finishedProductListDAO.saveItem(jsonToEntity(new JSONObject(request.getParameter("data"))));
    }

    @Override
    public String deleteItem(HttpServletRequest request) throws Exception {
        return finishedProductListDAO.deleteItem(jsonToEntity(new JSONObject(request.getParameter("data"))));
    }
}